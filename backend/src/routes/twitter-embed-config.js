...
router.put("/:serverId", isAuthenticated, async (req, res) => {
  try {
    ...
    if (existing.rows.length > 0) {
      ...
      result = await db.query(
        `INSERT INTO twitter_embed_config (server_id, webhook_repost_enabled, pruning_enabled, pruning_max_days, webhook_reply_notifications, notify_self_replies, suppress_original_embed, reaction_emoji)
